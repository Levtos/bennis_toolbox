"""Coordinator: wires HA state to the pure decision engine."""
...
from .engine import DeviceConfig, DeviceState, GlobalContext, Decision, evaluate

_LOGGER = logging.getLogger(__name__)


class BenniPlugCoordinator:
    """Reads HA state, calls the engine, exposes decisions and (optionally) drives switches."""
    ...
    async def async_evaluate_all(self, *, ha_just_started: bool = False) -> None:
        ...
        for cfg in self.configs.values():
            ...
            # Diffuser cycle bookkeeping: flip phase when we change desired
    ...
    async def _apply_decision(self, cfg: DeviceConfig, st: DeviceState, dec: Decision) -> None:
        ...
        try:
            await self.hass.services.async_call(
                "switch", target, {"entity_id": cfg.switch_entity}, blocking=False,
            )
            ...
            _LOGGER.info("plug_policy_engine: %s on %s — %s", target, cfg.switch_entity, dec.reason)
        except Exception as err:  # noqa: BLE001
            _LOGGER.error("plug_policy_engine: switch call failed for %s: %s", cfg.switch_entity, err)
