...
class TitleClassifierPanel extends HTMLElement {
  ...
  async _loadSources() {
    try {
      this._sources = await this._ws({ type: "bennis_toolbox/title_classifier/get_sources" });
  ...
  async _loadEntries({ showLoading = false, resetPage = false } = {}) {
    ...
    try {
      const message = { type: "bennis_toolbox/title_classifier/list_entries" };
  ...
  async _save(entryId, key, value, inputEl, buttonEl = null) {
    try {
      ...
      await this._ws({ type: "bennis_toolbox/title_classifier/update_entry", entry_id: entryId, key, enum_value: value });
  ...
  async _acSearch(query) {
    try {
      const msg = {
        type: "bennis_toolbox/title_classifier/list_entries",
        search: query,
        include_hidden: true,
        limit: 25,
      };
  ...
  async _hideUnmapped() {
    ...
    try {
      const res = await this._ws({ type: "bennis_toolbox/title_classifier/hide_unmapped", entry_id: this._filterSource });
