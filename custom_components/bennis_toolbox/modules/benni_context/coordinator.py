"""DataUpdateCoordinator für Benni Context.
...
Storage: `.storage/bennis_toolbox_benni_context_state_<entry_id>` via Toolbox-
Helper. Cross-Modul-Inputs (Wake Planner, Title Classifier) werden **nicht**
direkt importiert — sie kommen ausschließlich als konfigurierte HA-Entities
in den Coordinator.
"""
...
class BenniContextCoordinator(DataUpdateCoordinator[ComputedState]):
    """Drive every Benni-Context sensor from a single computation step."""
...
def coordinator_from_hass(hass: HomeAssistant, entry_id: str) -> BenniContextCoordinator | None:
...
def all_benni_context_coordinators(hass: HomeAssistant) -> list[BenniContextCoordinator]:
    out: list[BenniContextCoordinator] = []
